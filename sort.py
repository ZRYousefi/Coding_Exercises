...
def quicksort(arr):
    arr = quicksort_helper(arr, 0, len(arr) - 1)
    return arr

def quicksort_helper(arr, left, right):
    if left < right:
        partition_pose = partition(arr, left, right)
        arr = quicksort_helper(arr, left, partition_pose - 1)
        arr = quicksort_helper(arr, partition_pose + 1, right)
    return arr
